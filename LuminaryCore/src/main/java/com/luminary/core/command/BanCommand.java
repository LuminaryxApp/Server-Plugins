...
public class BanCommand implements CommandExecutor, TabCompleter {
    ...
    public BanCommand(LuminaryCore plugin) {
        this.plugin = plugin;
    }

    @Override
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        if (!sender.hasPermission("luminarycore.ban")) {
            MessageUtil.send(sender, plugin.getConfigManager().getMessage("no-permission"));
            return true;
        }

        if (args.length < 1) {
            MessageUtil.send(sender, "&cUsage: /ban <player> [reason]");
            return true;
        }
        ...
        if (target == null) {
            ...
            plugin.getModerationManager().ban(offlinePlayer.getUniqueId(), targetName, sender, reason, -1);
            MessageUtil.send(sender, plugin.getConfigManager().getMessage("ban-success", "{player}", targetName, "{reason}", reason));
            return true;
        }

        // Check if target has bypass permission
        if (target.hasPermission("luminarycore.ban.bypass")) {
            MessageUtil.send(sender, plugin.getConfigManager().getMessage("cannot-punish"));
            return true;
        }

        String reason = args.length > 1 ? String.join(" ", java.util.Arrays.copyOfRange(args, 1, args.length)) : "No reason provided";
        plugin.getModerationManager().ban(target.getUniqueId(), target.getName(), sender, reason, -1);
        MessageUtil.send(sender, plugin.getConfigManager().getMessage("ban-success", "{player}", target.getName(), "{reason}", reason));
    ...
    public List<String> onTabComplete(CommandSender sender, Command command, String alias, String[] args) {
        ...
        if (!sender.hasPermission("luminarycore.ban")) return completions;
