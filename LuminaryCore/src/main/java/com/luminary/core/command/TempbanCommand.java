...
public class TempbanCommand implements CommandExecutor, TabCompleter {
    ...
    public TempbanCommand(LuminaryCore plugin) {
        this.plugin = plugin;
    }

    @Override
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        if (!sender.hasPermission("luminarycore.tempban")) {
            MessageUtil.send(sender, plugin.getConfigManager().getMessage("no-permission"));
            return true;
        }

        if (args.length < 2) {
            MessageUtil.send(sender, "&cUsage: /tempban <player> <duration> [reason]");
        ...
        if (target == null) {
            ...
            plugin.getModerationManager().ban(offlinePlayer.getUniqueId(), targetName, sender, reason, duration);
            MessageUtil.send(sender, plugin.getConfigManager().getMessage("tempban-success",
                "{player}", targetName,
        ...
        String reason = args.length > 2 ? String.join(" ", Arrays.copyOfRange(args, 2, args.length)) : "No reason provided";
        plugin.getModerationManager().ban(target.getUniqueId(), target.getName(), sender, reason, duration);
        MessageUtil.send(sender, plugin.getConfigManager().getMessage("tempban-success",
    ...
    public List<String> onTabComplete(CommandSender sender, Command command, String alias, String[] args) {
        ...
        if (!sender.hasPermission("luminarycore.tempban")) return completions;
        ...
        if (args.length == 1) {
            ...
            completions.addAll(Arrays.asList("1h", "6h", "12h", "1d", "3d", "7d", "14d", "30d"));
        }
        return completions;
    }
}
